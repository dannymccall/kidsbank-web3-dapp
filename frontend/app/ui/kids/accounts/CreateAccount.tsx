...
import { addKidSchema } from "@/app/lib/definitions";
...
type FormData = z.infer<typeof addKidSchema>;
...
const CreateAccount = () => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const response = await makeRequest("/api/kids/accounts/createAccount", {
        method: "POST",
        body: JSON.stringify({...data, userId: user?.userId, email: user?.email}),
      });
      ...
      <div className="w-full max-w-md bg-white shadow-md rounded-lg p-6">
        ...
        {message.showMessage ? (
          <div className="text-center">
            ...
            <button className="btn btn-sm bg-[rgb(90,191,249)] text-white" onClick={() => setMessage({showMessage: false})}>Register another Kid</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Child Name */}
            <div>
              <label className="block font-semibold mb-1">Child Name</label>
              <div className="relative">
