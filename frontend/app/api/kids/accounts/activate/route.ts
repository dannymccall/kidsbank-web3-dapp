...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { childAddress, userId, email, role } = body;
    console.log(body);
    ...
    if (!kid.childName.trim())
      return NextResponse.json({
        success: false,
        message: "Address is not registered",
      });

    const estimateGas = await contract.methods
      .activateAccont(childAddress)
    ...
    const tx = await new Promise((resolve, reject) => {
      contract.methods
        .activateAccont(childAddress)
        .send({ from: account, gas: estimateGas as unknown as bigint | any })
